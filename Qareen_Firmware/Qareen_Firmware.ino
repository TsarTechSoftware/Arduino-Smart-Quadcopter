...
int ESCFreq[4] = {LEDFreq, LEDFreq, LEDFreq, 0};
...
void tempLEDSimulation(){
   ...
   if(truePitch > (inputPRYA[0] + errorDomain[0])){
        ESCFreq[0] = -truePitch/slowerMotors;
        ESCFreq[1] = -truePitch/slowerMotors;
        ESCFreq[2] = truePitch/fasterMotors;
        ESCFreq[3] = truePitch/fasterMotors;
   }else if(truePitch < (inputPRYA[0] - errorDomain[0])){
    //if(ESCFreq[3] <= maxMotorSpeed && ESCFreq[0] >= minMotorSpeed){
        ESCFreq[0] = -truePitch/fasterMotors;
        ESCFreq[1] = -truePitch/fasterMotors;
        ESCFreq[2] = truePitch/slowerMotors;
        ESCFreq[3] = truePitch/slowerMotors;
      //}
   }
   
  if(trueRoll > (inputPRYA[1] + errorDomain[1])){
    //if(ESCFreq[1] <= maxMotorSpeed && ESCFreq[0] >= minMotorSpeed){
        ESCFreq[0] = -trueRoll/slowerMotors;
        ESCFreq[1] = trueRoll/fasterMotors;
        ESCFreq[2] = trueRoll/fasterMotors;
        ESCFreq[3] = -trueRoll/slowerMotors;
     // }
  }else if(trueRoll < (inputPRYA[1] - errorDomain[1])){
    //if(ESCFreq[0] <= maxMotorSpeed && ESCFreq[1] >= minMotorSpeed){
        ESCFreq[0] = -trueRoll/fasterMotors;
        ESCFreq[1] = trueRoll/slowerMotors;
        ESCFreq[2] = trueRoll/slowerMotors;
        ESCFreq[3] = -trueRoll/fasterMotors;
      //}
  }
    analogWrite(ESCTwo,ESCFreq[2] + LEDFreq);
    analogWrite(ESCThree,ESCFreq[3] + LEDFreq);
    analogWrite(ESCZero,ESCFreq[0] + LEDFreq);
    analogWrite(ESCOne,ESCFreq[1] + LEDFreq);
}
